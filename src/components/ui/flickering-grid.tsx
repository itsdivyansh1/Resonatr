...
export const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const precomputedFillStyles = useMemo(() => {
    const steps = 10;
    return Array.from(
      { length: steps },
      (_, i) => `${memoizedColor}${((i + 1) / steps) * maxOpacity})`
    );
  ...
  const drawGrid = useCallback(
    (
      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          const styleIndex = Math.floor((opacity / maxOpacity) * 10);
          ctx.fillStyle =
            precomputedFillStyles[styleIndex] || `${memoizedColor}${opacity})`;
    ...
    [memoizedColor, precomputedFillStyles, squareSize, gridGap, maxOpacity]
  );
  ...
  return (
    <div
      ref={containerRef}
      className={cn("h-[100vh] w-full", className)}
      {...props}
    >
      <canvas
        ref={canvasRef}
